...
void crearTablero(Parcela parcelas[4][4]){
	 for(int fila=0;fila<5;fila++){
	   	for(int columna=0;columna<5;columna++){

	   		parcelas[fila][columna].estadoParcela=64;
    		...
    		(parcelas[fila][columna].necesitaRecuperacion)=NULL;
    		(parcelas[fila][columna].recuperacion)=10000;
    		(parcelas[fila][columna].regada)=NULL;
    		(parcelas[fila][columna].tipoSiembra)=64;
	    	}
	    }
}
...
bool verificarEstadoCosecha(int fila,int  columna, Parcela parcelas[4][4]){
	bool puedeUsarse;
...
void cambiarEstadoParcela(int fila, int columna, Parcela parcelas[4][4], char tipoSemillaMostrar){
	parcelas[fila][columna].estadoParcela=tipoSemillaMostrar;
	cout<<"Se cambio el estado de la parcela correctamente.\n";
}
void agregarDatosParcela(Parcela parcelas[4][4], int fila, int columna,
		char tipoSemilla){
	parcelas[fila][columna].tipoSiembra=tipoSemilla;

	parcelas[fila][columna].estaSembrada=true;
}
void agregarTurnosCosecha(Parcela parcelas[4][4], int fila, int columna){
...
void recuperacionParcela(Parcela parcelas[4][4], int fila, int columna, semillas *tipoA, semillas *tipoB, semillas *tipoC){
...
bool verificarParcelaSecaVacia(int fila, int columna, Parcela parcelas[4][4]){
...
void rutinaSembrar(int fila, int columna, estadoJugador *jugador,
		Parcela parcelas[4][4],semillas *tipoA, semillas *tipoB,
...
////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////7////regar///////////////////////////////////////////////////////////
bool verificoSiembra(Parcela parcelas[4][4],int  fila, int columna){
...
void rutinaRegar(Parcela parcelas[4][4], estadoJugador *jugador, int fila, int columna){
...
bool verificoCosechaParcela(Parcela parcelas[4][4],int fila, int columna){
...
void actualizarCosecha(Parcela parcelas[4][4], int fila, int columna){
	//(*(*(parcelas+fila)+columna)).estaPodrida=false;
...
void rentabilidad(int fila,int columna, Parcela parcelas[4][4],semillas *tipoA,
...
void agregarCosechadas(Parcela parcelas[4][4]){
...
void recuperarParcelasCosechadas(Parcela parcelas[4][4]){
...
void recuperacionPodrida(int fila, int columna, Parcela parcelas[4][4]){
...
void parcelasPodridas(Parcela parcelas[4][4]){
...
void buscoParcelas(Parcela parcelas[4][4], int fila, int columna){
...
void recuperarParcelaSecAnterior(Parcela parcelas[4][4],int fila, int columna){
...
void agregarParcelaSeca(Parcela parcelas[4][4]){
	for(int fila=0; fila<5;fila++){
		for(int columna=0; columna<5; columna++){
...
////////////////////////////////////////////////////////////////////////////////////////////////
bool menu(Parcela parcelas[4][4],estadoJugador *jugador,semillas *tipoA, semillas*tipoB,
		semillas *tipoC){
		bool seguir=true;
		...
		cout<<"Se intento volver a borrar"<<endl;
